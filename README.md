...
```bash
pip install "laya-openvino @ git+https://github.com/rupeshs/laya-openvino.git"
```

This replaces the upstream PyPI `laya` package if it is installed in the same environment.
...
On a CPU-only box the torch backend is the slow path. Export the checkpoint to OpenVINO IR once, then serve it with `OVAgent`:
...
agent = laya.OVAgent("laya-ov")          # no torch needed at inference time
result = agent.predict(state, questions)  # same payload as Agent.predict
```

`OVAgent` exposes the same `predict` / `system_one` contract as `Agent`, so it also drops into the router:

```python
from laya.router import Router

router = Router(preload=False)
router.attach("english", laya.OVAgent("laya-ov"))
```

The export directory is self-contained (IR, tokenizer and `rl_agent_config.json`), weights are compressed to fp16 by default (`--fp32` keeps full precision), and batch, sequence and option-count axes all stay dynamic. Measured on a 12th-gen Intel CPU, three questions over a short state: **301 ms torch → 180 ms OpenVINO (1.7x)**, with top answers unchanged and probabilities agreeing to within 0.0001. Your numbers will differ by CPU.

### Weight compression: fp16 vs int8 vs int4

The default fp16 export is a *storage* change only -- the math still runs in fp32 on a CPU without
native fp16. NNCF can go further and quantize the weights themselves. Built with
`research/scripts/quantize_ov.py`, then scored by `research/scripts/bench_quantization.py` on 90
preset questions (`choice`, `score` and `noul`) against the torch checkpoint, 12th-gen Intel CPU:
...
**int8 is the setting worth shipping**: half the size of fp16, 3.4x faster than torch on a single
question, and it still reproduces 98.9% of the torch top answers.

**int4 is not worth it.** It saves a further 162 MB, but it is *not* faster than int8 -- the two
are within noise on a single question, because both dequantize to the same compute path -- and it
is the only variant that changes answers often enough to notice.

Read the `dp` columns before trusting the agreement column: quantization disturbs the *probability*
much more than it disturbs the top answer. int4 moves a single probability by as much as 0.69, so
anything that thresholds one -- confidence gating, a `noul` cutoff, a shortlist margin -- degrades
faster than "93.3% agreement" suggests. On a control task that thresholds a single `noul` at 0.5,
fp16 and int8 both score 100% while every int4 config tested (asym and sym, group size 32 and 64,
ratio 0.8 and 1.0) lands between 35% and 65%. int4 collapses the gap between the two classes from
+0.77 to between +0.04 and +0.29, and a threshold has nothing left to sit in.
...
That copies `rl_agent_config.json` and `tokenizer/` across too, so `laya.OVAgent("laya-ov-int8")`
loads the result directly. (The script also shims an NNCF 2.19 bug that makes `compress_weights`
raise `TypeError: issubclass() arg 2 must be a class` on numpy >= 2.3.)
