...
# Modified by Rupesh Sreeraman, 2026: added OpenVINO backend support.
...
def to_internal(qdef: Dict) -> Dict:
    """Normalise a public question definition into the compact internal form."""
    t = qdef["type"]
...
def prepare_batch(tok, state, questions: Dict[str, Dict], max_len: int = 512, head_max_len: int = 192):
    """Tokenize every question over `state` into one collated batch.

    Returns `(qids, items, batch)`; `batch` is None only when `questions` is empty.
    """
    ...
    return qids, items, collate_items([items], tok.pad_token_id)
...
def decode_answers(
    ...
    """Turn raw model outputs into the public answer payload.

    Shared by every backend so temperature calibration and confidence are computed in exactly
    one place: a backend that recomputed them would be free to drift from the torch reference.
    """
    act = _softmax(act_logits, -1)
    answers = {}
